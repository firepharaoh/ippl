...
#define IPPL_ALVINE_SPECTRAL_ALVINESPECTRALOPS3D_HPP
    ...
    void computeSpectralVelocityModes3D() {
        ...
        double localMx = 0.0, localMy = 0.0, localMz = 0.0;
        double localKx = 0.0, localKy = 0.0, localKz = 0.0, localK2 = 0.0;
        double localVolume = 0.0;
        double localOxRe = 0.0, localOxIm = 0.0;
        double localOyRe = 0.0, localOyIm = 0.0;
        double localOzRe = 0.0, localOzIm = 0.0;
        double localUxRe = 0.0, localUxIm = 0.0;
        double localUyRe = 0.0, localUyIm = 0.0;
        double localUzRe = 0.0, localUzIm = 0.0;

        Kokkos::parallel_reduce(
            "debug_biot_savart_mode_3d",
            policy_type({nghost, nghost, nghost},
                        {static_cast<int>(ox.extent(0)) - nghost,
                         static_cast<int>(ox.extent(1)) - nghost,
                         static_cast<int>(ox.extent(2)) - nghost}),
            KOKKOS_LAMBDA(const int i, const int j, const int k,
                          double& mxOut, double& myOut, double& mzOut,
                          double& kxOut, double& kyOut, double& kzOut,
                          double& k2Out, double& volumeOut,
            ...
            Kokkos::Sum<double>(localMx),
            Kokkos::Sum<double>(localMy),
            Kokkos::Sum<double>(localMz),
            Kokkos::Sum<double>(localKx),
            Kokkos::Sum<double>(localKy),
            Kokkos::Sum<double>(localKz),
            Kokkos::Sum<double>(localK2),
            Kokkos::Sum<double>(localVolume),
            Kokkos::Sum<double>(localOxRe),
            Kokkos::Sum<double>(localOxIm),
            Kokkos::Sum<double>(localOyRe),
            Kokkos::Sum<double>(localOyIm),
            Kokkos::Sum<double>(localOzRe),
            Kokkos::Sum<double>(localOzIm),
            Kokkos::Sum<double>(localUxRe),
            Kokkos::Sum<double>(localUxIm),
            Kokkos::Sum<double>(localUyRe),
            Kokkos::Sum<double>(localUyIm),
            Kokkos::Sum<double>(localUzRe),
            Kokkos::Sum<double>(localUzIm));

        double mxDebug = 0.0, myDebug = 0.0, mzDebug = 0.0;
        double kxDebug = 0.0, kyDebug = 0.0, kzDebug = 0.0, k2Debug = 0.0;
        double volumeDebug = 0.0;
        double oxReDebug = 0.0, oxImDebug = 0.0;
        double oyReDebug = 0.0, oyImDebug = 0.0;
        double ozReDebug = 0.0, ozImDebug = 0.0;
        double uxReDebug = 0.0, uxImDebug = 0.0;
        double uyReDebug = 0.0, uyImDebug = 0.0;
        double uzReDebug = 0.0, uzImDebug = 0.0;

        ippl::Comm->allreduce(localMx, mxDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localMy, myDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localMz, mzDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localKx, kxDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localKy, kyDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localKz, kzDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localK2, k2Debug, 1, std::plus<double>());
        ippl::Comm->allreduce(localVolume, volumeDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localOxRe, oxReDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localOxIm, oxImDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localOyRe, oyReDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localOyIm, oyImDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localOzRe, ozReDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localOzIm, ozImDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localUxRe, uxReDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localUxIm, uxImDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localUyRe, uyReDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localUyIm, uyImDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localUzRe, uzReDebug, 1, std::plus<double>());
        ippl::Comm->allreduce(localUzIm, uzImDebug, 1, std::plus<double>());

        if (ippl::Comm->rank() == 0) {
            const bool firstWrite = !biot_savart_3d_debug_initialized_m;
            std::ofstream out(diagnosticFileName("biot_savart_3d_debug.csv"),
                              firstWrite ? std::ios::out : std::ios::app);
            out.precision(16);
            out.setf(std::ios::scientific, std::ios::floatfield);

            if (firstWrite) {
                out << "method,dt,step,time,mx,my,mz,kx,ky,kz,k2,volume,"
                    << "omega_x_re,omega_x_im,omega_y_re,omega_y_im,omega_z_re,omega_z_im,"
                    << "u_x_re,u_x_im,u_y_re,u_y_im,u_z_re,u_z_im\n";
            }

            out << method_m << "," << dt_m << "," << it_m << "," << time_m << ","
                << mxDebug << "," << myDebug << "," << mzDebug << ","
                << kxDebug << "," << kyDebug << "," << kzDebug << ","
                << k2Debug << "," << volumeDebug << ","
                << oxReDebug << "," << oxImDebug << ","
                << oyReDebug << "," << oyImDebug << ","
                << ozReDebug << "," << ozImDebug << ","
            ...
            Inform m("biot_savart_3d_debug ");
            m << "mode=(" << mxDebug << "," << myDebug << "," << mzDebug
              << "), k=(" << kxDebug << "," << kyDebug << "," << kzDebug
              << "), k2=" << k2Debug << ", volume=" << volumeDebug
              << ", uHat=(" << uxReDebug << "," << uxImDebug << "),("
              << uyReDebug << "," << uyImDebug << "),("
              << uzReDebug << "," << uzImDebug << ")" << endl;
        }

        biot_savart_3d_debug_initialized_m = true;
    }
