...
The `--integrator euler` path follows the eight-step IMEX pipeline in
`Screenshot 2026-09-11 at 17.51.02 (2)(1).png`. Comments in
`spectral/SFSLEuler3D.hpp` identify the diagram's steps. Leapfrog still uses its previous algorithm. RK4
uses the grid-only Strang adaptation described below.

Stretching is evaluated using omega^n and grad(u^n) on the original IFFT grid
and transformed back with FFT. These source modes are retained across
advection. For stored `q = omega_hat / k^2`, the final update is

```
q_new = (q_advection_only + dt * stretching_hat_old / k^2) / (1 + nu * dt * k^2)
```

The existing zero-mode, Nyquist, filtering, and Biot-Savart projection
conventions are retained, with filtering/projection after the combined update.
IFFT fields supply OLD velocity u^n and strengths Gamma^n to cell-center
particles. Euler transport moves positions but does not change strengths.
Type-1 NUFFT scatters those advected particles, then the saved stretching
and implicit viscosity update the spectral state. Final velocity is rebuilt
and particles are discarded and replaced on the lattice using IFFT sampling.
No CIC is used. Direct sampling requires `nx = ny = nz` and
`np = nx * ny * nz`.

This is first-order IMEX Euler, with backward Euler for diffusion. It
...
Reordering source and transport changes floating-point results and finite-dt
errors compared with the previous source-first pipeline. No extra device
buffers or production host copies are needed; source buffers survive the
NUFFT scatter. FFT/NUFFT transfer and aliasing errors still limit accuracy;
using Type-1 NUFFT does not inherently remove aliasing of sampled products.
...
- A complete TGV timestep matches an independent direct Fourier sum of
  analytically advected old strengths plus stretching at ORIGINAL positions,
  followed by backward-Euler diffusion and the existing projection. This
  explicitly distinguishes advection-first from the old source-first path.
  Host-side DFT and mirror copies are restricted to the small regression test.
- Three full timesteps of a transverse shear preserve the final IMEX state
  through destruction and IFFT resampling. Its energy
